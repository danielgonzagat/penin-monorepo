...
INDEX_P  = Path(os.getenv("ET_INDEX", "/opt/et_replica1/index/etomega.faiss"))
META_P   = Path(os.getenv("ET_META",  "/opt/et_replica1/index/etomega_meta.json"))
PROMPT_P = Path(os.getenv("ET_PROMPT", "/opt/et_replica1/prompts/et_omega.txt"))
REPO_DIR = Path(os.getenv("ET_REPO",   "/opt/et_replica1/equation"))
LOGFILE  = os.getenv("ET_LOG", "/var/log/et_replica1/evolver.log")
...
def loop():
    ...
    if not INDEX_P.exists() or not META_P.exists():
        log.error("Índice RAG não encontrado. Gere /opt/et_replica1/index/etomega.faiss e meta.")
        return
